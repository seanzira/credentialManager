...
const Register = () => {
    ...
    const [ous, setOus] = useState([]);  // State for storing OUs
    ...
    useEffect(() => {
        const fetchOUs = async () => {
            try {
                ...
                setOus(response.data.ous);  // Store the list of OUs in state
            } catch (error) {
        ...
        const fetchDivisions = async () => {
            try {
                const response = await axios.get('http://localhost:3001/api/credentials/division-list');  
                setDivisions(response.data.divisions);  // Store the list of divisions
            } catch (error) {
